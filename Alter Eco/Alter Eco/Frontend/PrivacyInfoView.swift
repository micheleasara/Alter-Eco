    ...
    public var body: some View {
        ScrollView {
            VStack(alignment: .leading) {
                contact.padding()
                Spacer()
                privacyAndData.padding(.horizontal)
                Spacer()
                credits.padding(.horizontal)
            }
        }
    }
    ...
    private var contact: some View {
        VStack {
            Text("Feedback")
                .bold()
            VStack(alignment: .leading) {
                Text("All feedback is welcome! If you have suggestions, bug reports or anything similar, please drop us an e-mail at:")
                    .fontWeight(.regular)
                mailButton
